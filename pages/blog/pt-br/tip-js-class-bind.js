...
export default function Post() {
  return (
    ...
    >
      ...
      <BlogParagraph>
        Essa forma de design de código é muito comum, é o que chamamos de inversão de controle. Basicamente deixamos que o método <code>observer#subscribe</code> execute o método <code>Chat#handleMessage</code> quando bem entender.
      </BlogParagraph>
      ...
      <BlogParagraph>
        O que isso significa? Sempre que uma classe é definida em JavaScript, cada método dessa classe é encarado como uma função usando a sintaxe clássica de funções (usando a palavra chave <code>function</code>). É dessa forma que temos acesso ao <code>this</code> dentro de cada método (lembre-se, arrow functions não tem um <code>this</code> próprio, mas podem acessar o contexto lexico mais próximo de seu contexto).
      </BlogParagraph>

      <BlogParagraph>
        O problema é que engines que executam JavaScript não criam o contexto de execução dessas funções até que a função seja executada. Executando o código acima no <code>deno</code> temos o seguinte erro:
      </BlogParagraph>
...
error: Uncaught TypeError: Cannot read property 'print' of undefined
    ...
    at handleTimerMacrotask (deno:cli/rt/11_timers.js:302:7)`}
      ...
      <BlogParagraph>
       Note que existe um nome <code>this</code> no contexto da execução, porém o seu valor é <code>undefined</code>. Já era esperado o nome <code>this</code> existir já que ele é criado durante o <i>parsing</i> do programa.
      </BlogParagraph>

      <BlogSubtitle>Entendendo o problema</BlogSubtitle>

      <BlogParagraph>
        O motivo do valor do <code>this</code> ser <code>undefined</code> é porque o callback (<code>cb</code>) foi chamado fora do escopo da classe <code>Chat</code>. Lembre-se, a engine vai esperar até o último momento para criar os contextos de execução das funções. O V8, SpiderMonkey ou qualquer outra engine que executa JavaScript não pode se dar ao luxo de criar o escopo de todas as funções antes de começar a execução de programas JavaScript.
      </BlogParagraph>

      <BlogParagraph>
        Quando nós passamos um método de uma classe para um controle externo, eventualmente, chamar esse método; precisamos indicar explicitamente ao JavaScript para anexar a esse método o contexto da classe antes de passar a função a diante. Engines JavaScript não criam links entre métodos e classes automaticamente. Esse processo é custoso em termos de memória e processamento.
      </BlogParagraph>

      <BlogParagraph>
        Dessa forma, sempre que um callback que dependende de um <code>this</code> é passado para um executor externo e não é feito nenhum anexo de contexto nessa função, o valor do <code>this</code> não será definido.
      </BlogParagraph>

      <BlogParagraph>
        Você pode simular o mesmo esquema em uma classe bem mais simples, veja o seguinte:
      </BlogParagraph>
...
new A().run()`}
      ...
      <BlogParagraph>
        Executando o código acima com o deno, vemos o mesmo erro ocorrer novamente.
      </BlogParagraph>
...
error: Uncaught TypeError: Cannot read property 'name' of undefined
    ...
    at gustavosantos.dev/code/blog/tip-js-class-bind/example_class_a.js:13:9`}
      ...
      <BlogParagraph>
        O problema é o mesmo que enfrentamos anteriormente. O executor externo nesse caso é o método <code>map</code>. Quando a função <code>A#handle</code> é passada para o <code>map</code>, o contexto de execução de <code>handle</code> só será criado quando for executado e, novamente, por padrão, o valor do <code>this</code> não será definido automaticamente.
      </BlogParagraph>
...
`}
      ...
      <BlogParagraph>
        Você pode obter o mesmo resultado usando <i>arrow functions</i>. Dessa forma, no momento da definição da arrow function, o escopo de execução daquela função vai ser criado e anexado a função. Assim, quando o callback for executado pelo controle externo, o <code>this</code> já vai estar populado corretamente. Veja o exemplo abaixo.
      </BlogParagraph>
      ...
      <BlogParagraph>
        Por curiosidade, podemos fazer o mesmo com a classe <code>A</code> definida em <code>example_class_a.js</code>:
      </BlogParagraph>
...
3 : class A`}
      ...
      <BlogParagraph>
        Quando você executa o método <code>bind</code> passando um objeto (geralmente o <code>this</code>), é devolvido uma função cujo contexto já está definido. Assim, você pode passar essa função para um executor externo sem se preocupar com problemas do tipo <code>Cannot read property '*' of undefined</code>.
      </BlogParagraph>

      <BlogParagraph>
        JavaScript definitivamente não é a melhor linguagem para se praticar OOP, porém após entender como coisas tipo <code>this</code> ou contextos de execução funcionam, definitivamente temos mais confiança em organizar nosso código em classes, que definitivamente é uma forma muito conveniente de encapsulamento.
      </BlogParagraph>
    </BlogPage>
  );
}
